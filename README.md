...
`SINGA` is a bioinformatics analysis pipeline designed for the assembly and variant calling of Monkeypox Virus (MPXV) samples. This pipeline prioritizes quality control by ensuring that low-quality reads are filtered out, primers are effectively trimmed, and human-derived reads are depleted, resulting in high-quality MPXV sequences. This repository is dedicated for the `Illumina` reads analysis.
...
SINGA is a bioinformatics analysis pipeline designed for the assembly and variant calling of Monkeypox Virus (MPXV) samples. This pipeline prioritizes quality control by ensuring that low-quality reads are filtered out, primers are effectively trimmed, and human-derived reads are depleted, resulting in high-quality MPXV sequences.

The pipeline supports both Illumina and Nanopore sequencing data, specifically tailored for amplicon-based approaches. For Illumina short-reads, it can analyze metagenomics data derived from shotgun sequencing or enrichment-based library preparation methods. In the case of Nanopore sequencing, the pipeline focuses on amplicon data generated using primer sets curated by the ARTIC Network.

Upon each release, automated continuous integration tests execute the pipeline on comprehensive datasets within AWS cloud infrastructure. This guarantees the pipeline's functionality on AWS, establishes sensible resource allocation defaults for real-world datasets, and allows for the persistent storage of results to benchmark different pipeline versions and comparative analysis sources. Results from these extensive tests, run for each platform option, are accessible on the SINGA website, with output directories named accordingly (e.g., platform_illumina/ and platform_nanopore/).

SINGA is built using Nextflow, a robust workflow management tool that facilitates task execution across various compute infrastructures in a highly portable manner. Utilizing Docker and Singularity containers, the installation process is simplified, ensuring reproducible results. The Nextflow DSL2 implementation of this pipeline employs one container per process, making it easier to manage and update software dependencies. Wherever feasible, these processes are submitted to and installed from the nf-core/modules repository, making them accessible to all nf-core pipelines and the broader Nextflow community!
...
# MPXV-SINGA : MonkeyPox Virus - Sequence Information from Nucleotide Generation and Assembly
